...
async function firestoreUpdateUserData(userId) {
  await fetch(`http://localhost:3000/api/user/${userId}/set_data/route`, {
      method: "POST",
...
async function firestoreGetUserData(userId) {
  await fetch(`http://localhost:3000/api/user/${userId}/get_data/route`, {
      method: "GET"
  })
  .then(res => res.json())
...
async function firestoreCheckUserInvestments(userId) {
  var new_investments = []
    await fetch(`http://localhost:3000/api/user/${userId}/investment_checks/route`, {
        method: "GET"
    })
